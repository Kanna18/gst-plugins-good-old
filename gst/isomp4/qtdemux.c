...
gst_qtdemux_loop_state_mmt_header(GstQTDemux * qtdemux);
...
static GstStaticPadTemplate gst_qtdemux_sink_template =
    ...
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS ("video/x-h265; audio/x-m4a; "
        "application/x-3gp")
    );
...
static GstFlowReturn gst_qtdemux_process_adapter (GstQTDemux * demux,
    gboolean force);
static GstFlowReturn bkpgst_qtdemux_process_adapter (GstQTDemux * demux,
    gboolean force);

//gboolean qtdemux_parse_moov (GstQTDemux * qtdemux,
  //  const guint8 * buffer, guint length);
static gboolean qtdemux_parse_node (GstQTDemux * qtdemux, GNode * node,
    const guint8 * buffer, guint length);
//gboolean qtdemux_parse_tree (GstQTDemux * qtdemux);
...
gst_qtdemux_init (GstQTDemux * qtdemux)
{
  ...
  qtdemux->moov_detected = 0;
  qtdemux->mdatoffset = -1;
  qtdemux->mmdatbuffer[0] = NULL;
  qtdemux->mmdatbuffer[1] = NULL;
  qtdemux->mdatbuffer = NULL;
  qtdemux->mainbuf = NULL;
        ...
        if (fmt == GST_FORMAT_TIME) {
          ...
          if (!qtdemux->pullbased) {
            ...
            if (gst_pad_peer_query (qtdemux->sinkpad, q)) {
              ...
              GST_DEBUG_OBJECT (qtdemux, "upstream BYTE seekable %d", seekable);
            }
            gst_query_unref (q);
          }
...
parse_failed:
  {
    GST_DEBUG_OBJECT (qtdemux, "Parsing of index %u failed!", index + 1);
    return -1;
  }
}
...
parse_failed:
  {
    GST_DEBUG_OBJECT (qtdemux, "Parsing of index %u failed!", new_index);
    return -1;
  }
}
    ...
    case GST_EVENT_SEGMENT:
    {
      ...
      if (segment.format == GST_FORMAT_BYTES) {
        ...
        gst_qtdemux_process_adapter (demux, TRUE);
		  if(0)
				bkpgst_qtdemux_process_adapter (demux, TRUE);
      } else {
...
void
gst_qtdemux_reset_stream (GstQTDemux * qtdemux)
{
	qtdemux->n_streams = 0;
	qtdemux->n_video_streams = 0;
	qtdemux->n_audio_streams = 0;
...
void
qtdemux_parse_mmpu (GstQTDemux * qtdemux, const guint8 * buffer, gint length)
{
	 guint8 bitfield;
  /* counts as header data */
  qtdemux->header_size += length;
  ...
  /* only consider at least a sufficiently complete ftyp atom */
	 ...
	 qtdemux->is_complete = bitfield & 0x01;
	 qtdemux->is_adc_present = bitfield & 0x02;
	 qtdemux->reserved = bitfield & 0xFC;
	 buffer +=1;
	 qtdemux->mpu_sequence_number = QT_UINT32((guint32*)buffer);
	 buffer +=4;
	 qtdemux->asset_id_scheme = QT_UINT32((guint32*)buffer);
	 buffer +=4;
	 qtdemux->asset_id_length = QT_UINT32((guint32*)buffer);
    GST_DEBUG_OBJECT (qtdemux,
          ...
          qtdemux->is_complete, qtdemux->is_adc_present, 
		  qtdemux->mpu_sequence_number, qtdemux->asset_id_scheme, qtdemux->asset_id_length);
#if 0
...
static void
qtdemux_parse_mmth (GstQTDemux * qtdemux, const guint8 * buffer, gint length)
{
  /* counts as header data */
  qtdemux->header_size += length;
...
#if 0
static void
qtdemux_parse_mmthsample (GstQTDemux * qtdemux, const guint8 * buffer, gint length)
{
  /* counts as header data */
  qtdemux->header_size += length;
  ...
  if (length >= 23) {
    ...
    qtdemux->Sequence_Number = QT_UINT32((guint32*) buffer);;
    GST_DEBUG_OBJECT (qtdemux, "Sequence_Number: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->Sequence_Number));
    buffer +=4;   

   qtdemux->trackrefindex = QT_UINT8((guint8*) buffer);
    GST_DEBUG_OBJECT (qtdemux, "TrackRefIndex: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->trackrefindex));
    buffer +=1;

   qtdemux->movie_fragemnt_sequence_number = QT_UINT8((guint32*) buffer);
    GST_DEBUG_OBJECT (qtdemux, "movie_fragemnt_sequence_number: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->movie_fragemnt_sequence_number));
    buffer +=4;

   qtdemux->SampleNumber = QT_UINT8((guint8*) buffer);
    GST_DEBUG_OBJECT (qtdemux, "SampleNumber: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->SampleNumber));
    buffer +=1;

   qtdemux->priority = QT_UINT8((guint8*) buffer);
    GST_DEBUG_OBJECT (qtdemux, "priority: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->priority));
    buffer +=1;

   qtdemux->dependency_counter = QT_UINT8((guint8*) buffer);
    GST_DEBUG_OBJECT (qtdemux, "dependency_counter: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->dependency_counter));
    buffer +=1;

   qtdemux->MFU_offset = QT_UINT32((guint32*) buffer);
    GST_DEBUG_OBJECT (qtdemux, "offset: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->MFU_offset));
    buffer +=4;

  qtdemux->length = QT_UINT32((guint32*) buffer);
  GST_DEBUG_OBJECT (qtdemux, "length: %" GST_FOURCC_FORMAT,
  GST_FOURCC_ARGS (qtdemux->length));
   
  }
}
#endif
static void
qtdemux_parse_hmhd (GstQTDemux * qtdemux, const guint8 * buffer, gint length)
{
  /* counts as header data */
  qtdemux->header_size += length;
  ...
  if (length >= 23) {
    ...
    qtdemux->maxPDUsize = QT_UINT16((guint16*) buffer);;
    GST_DEBUG_OBJECT (qtdemux, "maxPDUsize: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->maxPDUsize));
    buffer +=2;   

   qtdemux->avgPDUsize = QT_UINT16((guint16*) buffer);
    GST_DEBUG_OBJECT (qtdemux, "avgPDUsize: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->avgPDUsize));
    buffer +=2;

   qtdemux->maxbitrate = QT_UINT8((guint32*) buffer);
    GST_DEBUG_OBJECT (qtdemux, "maxbitrate: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->maxbitrate));
    buffer +=4;

   qtdemux->avgbitrate = QT_UINT32((guint32*) buffer);
    GST_DEBUG_OBJECT (qtdemux, "avgbitrate: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->avgbitrate));
    buffer +=4;

   qtdemux->hmhdreserved = QT_UINT32((guint32*) buffer);
    GST_DEBUG_OBJECT (qtdemux, "reserved: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (qtdemux->hmhdreserved));
    ...
    guint offset)
{
  ...
  if (ss_info->crypto_info) {
    GST_DEBUG_OBJECT (qtdemux, "unreffing existing crypto_info");
  ...
  for (i = 0; i < qtdemux->cenc_aux_sample_count; ++i) {
    ...
    if (uses_sub_sample_encryption) {
      ...
      GST_DEBUG_OBJECT (qtdemux, "subsample count: %u", n_subsamples);
    ...
    guint32 * ds_duration, guint32 * ds_size, guint32 * ds_flags)
{
  if (!stream->parsed_trex && qtdemux->moov_node) {
    ...
    GstByteReader trex_data;
	 GST_DEBUG_OBJECT(qtdemux, "AAAAAAAAAAA not parsed and moov_node");
...
check_update_duration (GstQTDemux * qtdemux, GstClockTime duration)
{
  ...
  for (i = qtdemux->mediaType==1?0:1; i < qtdemux->n_streams; i++) {
    QtDemuxStream *stream = qtdemux->streams[i];
    if (stream) {
    ...
    gboolean has_tfdt)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "parsing trun stream %d; "
  ...
  if (flags & TR_DATA_OFFSET) {
    ...
    if (!gst_byte_reader_get_int32_be (trun, &data_offset))
      goto fail;
    GST_DEBUG_OBJECT (qtdemux, "trun data offset %d", data_offset);
    /* default base offset = first byte of moof */
    if (*base_offset == -1) {
      GST_DEBUG_OBJECT (qtdemux, "base_offset at moof");
    ...
    *running_offset = *base_offset + data_offset;
    ...
    if (*base_offset == -1) {
      *base_offset = moof_offset + moof_length + 8;
      GST_DEBUG_OBJECT (qtdemux, "base_offset assumed in mdat after moof");
      ismv = TRUE;
    }
    if (*running_offset == -1)
  ...
  GST_DEBUG_OBJECT (qtdemux, "running offset now %" G_GINT64_FORMAT,
      *running_offset);
  GST_DEBUG_OBJECT (qtdemux, "trun offset %d, flags 0x%x, entries %d",
  ...
  if (flags & TR_FIRST_SAMPLE_FLAGS) {
    if (G_UNLIKELY (flags & TR_SAMPLE_FLAGS)) {
      ...
      if (!gst_byte_reader_get_uint32_be (trun, &first_flags))
        goto fail;
      GST_DEBUG_OBJECT (qtdemux, "first flags: 0x%x", first_flags);
    }
  }
  ...
  if (flags & TR_SAMPLE_DURATION) {
    GST_DEBUG_OBJECT (qtdemux, "entry duration present");
    dur_offset = entry_size;
    entry_size += 4;
  }
  if (flags & TR_SAMPLE_SIZE) {
    GST_DEBUG_OBJECT (qtdemux, "entry size present");
    size_offset = entry_size;
    entry_size += 4;
  }
  if (flags & TR_SAMPLE_FLAGS) {
    GST_DEBUG_OBJECT (qtdemux, "entry flags present");
    flags_offset = entry_size;
    entry_size += 4;
  }
  if (flags & TR_COMPOSITION_TIME_OFFSETS) {
    ct_offset = entry_size;
    GST_DEBUG_OBJECT (qtdemux, "entry ct offset present: %u", ct_offset);
    entry_size += 4;
  }
  ...
  GST_DEBUG_OBJECT (qtdemux, "%u allocating n_samples %u * %u (%.2f MB)",
  ...
  if (qtdemux->fragment_start != -1) {
    timestamp = GSTTIME_TO_QTSTREAMTIME (stream, qtdemux->fragment_start);
	 GST_DEBUG_OBJECT (qtdemux,"TTTIME");
    qtdemux->fragment_start = -1;
  } else {
    if (stream->n_samples == 0) {
      if (decode_ts > 0) {
	 ...
	 GST_DEBUG_OBJECT (qtdemux,"TTTIME last dur moof %"G_GINT64_FORMAT" stream timestamp: %"G_GUINT64_FORMAT" && stream duration: %"G_GUINT32_FORMAT" && timestamp: %"G_GINT64_FORMAT, stream->duration_last_moof, stream->samples[stream->n_samples - 1].timestamp, stream->samples[stream->n_samples - 1].duration, timestamp);
  ...
  for (i = 0; i < samples_count; i++) {
    ...
    sample->duration = dur;
	 ...
	 if(qtdemux->mediaType == 1)
	 GST_DEBUG_OBJECT(qtdemux, "OXXXXX sample_number %"G_GUINT32_FORMAT" sample timestamp %"G_GINT64_FORMAT " PTS offset: %"G_GUINT32_FORMAT" sample offset: %"G_GUINT64_FORMAT" sample size: %"G_GUINT32_FORMAT" sample duration: %"G_GUINT32_FORMAT, sample->sample_number, sample->timestamp, sample->pts_offset, sample->offset, sample->size, sample->duration);
	 else
	 GST_DEBUG_OBJECT(qtdemux, "IXXXXX sample_number %"G_GUINT32_FORMAT" sample timestamp %"G_GINT64_FORMAT " PTS offset: %"G_GUINT32_FORMAT" sample offset: %"G_GUINT64_FORMAT" sample size: %"G_GUINT32_FORMAT" sample duration: %"G_GUINT32_FORMAT, sample->sample_number, sample->timestamp, sample->pts_offset, sample->offset, sample->size, sample->duration);
	 //GST_DEBUG_OBJECT(qtdemux, "IXXXXX sample timestamp %"G_GINT64_FORMAT "sample offset: %"G_GUINT64_FORMAT" sample size: %"G_GUINT32_FORMAT" sample duration: %"G_GUINT32_FORMAT, sample->timestamp, sample->offset, sample->size, sample->duration);
  ...
  GST_DEBUG_OBJECT(qtdemux, "LLLLLLLLLLL media type: %d stream samples: %d, mdatoffset:%"G_GUINT64_FORMAT", mdatoffset:%"G_GUINT64_FORMAT", mdatsize:%"G_GUINT64_FORMAT, qtdemux->mediaType, stream->n_samples, qtdemux->mdatleft, qtdemux->mdatoffset, qtdemux->mdatsize);
    ...
    gint64 * base_offset)
{
  ...
  if (!gst_byte_reader_get_uint32_be (tfhd, &track_id))
    goto invalid_track;
	 ...
	 GST_INFO_OBJECT (qtdemux, "MMMMMMMMOOF stream with track id %i  mediaType: %d",
        track_id, qtdemux->mediaType);
#if 1
  //if(track_id == 2 && qtdemux->ismmth)
//	 goto invalid_track;
  ...
  *stream = qtdemux_find_stream (qtdemux, track_id);
  ...
  if (flags & TF_DEFAULT_SAMPLE_FLAGS)
    if (!gst_byte_reader_get_uint32_be (tfhd, default_sample_flags))
      goto invalid_track;
  GST_DEBUG_OBJECT (qtdemux, " trak id:%u, base_offset: %"G_GUINT64_FORMAT", default_sample_duration: %u, default_sample_size: %u, default_sample_flags: %u", track_id, *base_offset, *default_sample_duration, *default_sample_size, *default_sample_flags );
...
//TODO: BY SAFWAN, DO NOT FLUSH
  ...
  while (pssh_node) {
    GST_DEBUG_OBJECT (qtdemux, "Parsing pssh box.");
...
qtdemux_parse_tfra (GstQTDemux * qtdemux, GNode * tfra_node)
{
  ...
  for (i = 0; i < num_entries; i++) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "fragment time: %" GST_TIME_FORMAT ", "
...
gst_qtdemux_loop_state_mmt_header(GstQTDemux * qtdemux)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW loop at position %" G_GUINT64_FORMAT " %x, state %s",
      cur_offset, (int)cur_offset, qt_demux_state_string (qtdemux->state));
  ...
  if (G_LIKELY (map.size >= 8)){
    PayldrLength=ParseMMTPHDR(map.data);
		 gst_buffer_unmap (buf, &map);
		 gst_buffer_unref (buf);
		 buf = NULL;
		 //GST_DEBUG_OBJECT (qtdemux, "SKIP BYTES offset:%d, payload length: %"G_GUINT64_FORMAT, offset, PayldrLength);
	 if((PayldrLength == 3) || (PayldrLength <= 1))
	 {
		 GST_DEBUG_OBJECT (qtdemux, "SKIP BYTES offset:%d, payload length: %"G_GUINT64_FORMAT, offset, PayldrLength);
		 //qtdemux->offset +=363;
		 //qtdemux->offset +=332;
		 ret = gst_pad_pull_range (qtdemux->sinkpad, qtdemux->offset, 4000, &buf);
		 if (G_UNLIKELY (ret != GST_FLOW_OK))
		 {
			 GST_DEBUG_OBJECT (qtdemux, "EEEEEEEEEEEEEEEError while fetching buffer");
			 return GST_FLOW_EOS;
		 }
		 gst_buffer_map (buf, &map, GST_MAP_READ);
		 qtdemux->offset +=skip_bytes(map.data, 4000);
		 gst_buffer_unmap (buf, &map);
		 gst_buffer_unref (buf);
		 return GST_FLOW_OK;
	 }
	 if(PayldrLength != 0x2)
	 {
		// packet_offset = offset-(PayldrLength+2);
		 qtdemux->packet_offset = qtdemux->offset+ 14;
		 GST_DEBUG_OBJECT (qtdemux, "AV_PACKET offset:%d, payload length: %"G_GUINT64_FORMAT, offset, PayldrLength);
	   qtdemux->ref_offset = qtdemux->packet_offset;

		 ret = gst_pad_pull_range (qtdemux->sinkpad, qtdemux->packet_offset, PayldrLength, &buf);
		 if (G_UNLIKELY (ret != GST_FLOW_OK))
		 {
			 GST_DEBUG_OBJECT (qtdemux, "EEEEEEEEEEEEEEEError while fetching buffer");
		    return GST_FLOW_EOS;
		 ...
		 gst_buffer_map (buf, &map, GST_MAP_READ);
		 if(PayldrLength !=4)
			  ret = ParseMMTPV1Pack(qtdemux, map.data, NULL);

		 gst_buffer_unmap (buf, &map);
		 gst_buffer_unref (buf);
		  if(ret == -1)
			 GST_DEBUG_OBJECT (qtdemux, "EEEEEEEEEEEEEEEError while fetching buffer");
		 qtdemux->offset +=PayldrLength+16; 
	 }
	 else
	 {
#if 1 
		 ret = gst_pad_pull_range (qtdemux->sinkpad, qtdemux->offset, 8192, &buf);
		 if (G_UNLIKELY (ret != GST_FLOW_OK))
		 {
			 GST_DEBUG_OBJECT (qtdemux, "EEEEEEEEEEEEEEEError while fetching buffer");
		    return GST_FLOW_EOS;
		 ...
		 gst_buffer_map (buf, &map, GST_MAP_READ);
		 //SigOffset =ParseSignal(map.data,8192);
		 SigOffset =ParseSignal(map.data);
		 GST_DEBUG_OBJECT (qtdemux, "Bitstream offset: %"G_GUINT64_FORMAT "NNNNNNNNNNNNNN offset:%d",  qtdemux->offset, SigOffset);
		 qtdemux->offset +=SigOffset;

		 //length = ParseMMTPHDR(map.data);
		 GST_DEBUG_OBJECT (qtdemux, " extract_initial_length_mmt length %"G_GUINT64_FORMAT, PayldrLength);
		 gst_buffer_unmap (buf, &map);
		 gst_buffer_unref (buf);
	 }
#endif
...
gst_qtdemux_loop_state_header (GstQTDemux * qtdemux)
{
  ...
  if (G_UNLIKELY (ret != GST_FLOW_OK))
    goto beach;
  gst_buffer_map (buf, &map, GST_MAP_READ);
  GST_DEBUG_OBJECT (qtdemux, "SAFF extract_initial_length_and_fourcc");
  if (G_LIKELY (map.size >= 8))
    extract_initial_length_and_fourcc (map.data, map.size, &length, &fourcc);
  gst_buffer_unmap (buf, &map);
  gst_buffer_unref (buf);
  ...
  switch (fourcc) {
    ...
    case FOURCC_mmpu:
    {
      ...
      qtdemux->is_mmpu = 1;
       ...
       GST_DEBUG_OBJECT (qtdemux, "SAFISDA: Skipping mmpu atom for now..");
      /* extract major brand; might come in handy for ISO vs QT issues */
      ret = gst_qtdemux_pull_atom (qtdemux, cur_offset, length, &mmpu);
      if (ret != GST_FLOW_OK)
        goto beach;
      qtdemux->offset += length;
      gst_buffer_map (mmpu, &map, GST_MAP_READ);
      qtdemux_parse_mmpu (qtdemux, map.data, map.size);
   ...
   case FOURCC_mmth:
    {
      ...
      ret = gst_qtdemux_pull_atom (qtdemux, cur_offset, length, &mmth);
      if (ret != GST_FLOW_OK)
        goto beach;
      qtdemux->offset += length;
      gst_buffer_map (mmth, &map, GST_MAP_READ);
      qtdemux_parse_mmth (qtdemux, map.data, map.size);
...
gst_qtdemux_seek_to_previous_keyframe (GstQTDemux * qtdemux)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "keyframe index %u ts %" G_GUINT64_FORMAT
    ...
    gboolean * keyframe)
{
  ...
  if (G_UNLIKELY (time_position == GST_CLOCK_TIME_NONE))
	 {
	 GST_DEBUG_OBJECT (qtdemux, "GST_CLOCK_TIME_NONE::::::::::::::::");
    goto eos;
	 }

  seg_idx = stream->segment_index;
	 GST_DEBUG_OBJECT (qtdemux, "SSEGMENT: %d", seg_idx);
  ...
  if (G_UNLIKELY (QTSEGMENT_IS_EMPTY (&stream->segments[stream->
    ...
    GST_DEBUG_OBJECT (qtdemux, "Empty segment activated,"
  ...
  *empty = FALSE;
  ...
  GST_DEBUG_OBJECT (qtdemux, "segment active, index = %u of %u",
  ...
  if (!qtdemux_parse_samples (qtdemux, stream, stream->sample_index)) {
    GST_DEBUG_OBJECT (qtdemux, "Parsing of index %u failed!",
        stream->sample_index);
    return FALSE;
  }

  /* now get the info for the sample we're at */
  GST_DEBUG_OBJECT(qtdemux, "SAMPLE_INDEX: %"G_GUINT32_FORMAT, stream->sample_index);
  ...
  *keyframe = QTSAMPLE_KEYFRAME (stream, sample);
   GST_DEBUG_OBJECT(qtdemux, "VVVVVVVVV time_stamp %"G_GUINT64_FORMAT " pts offset %"G_GUINT32_FORMAT " PTS %"G_GUINT64_FORMAT " DTS %"G_GUINT64_FORMAT " cslg_shift: %"G_GUINT32_FORMAT, sample->timestamp, sample->pts_offset, QTSAMPLE_PTS (stream, sample), QTSAMPLE_DTS (stream, sample), (stream)->cslg_shift);
...
gst_qtdemux_advance_sample (GstQTDemux * qtdemux, QtDemuxStream * stream)
{
  ...
  if (QTSAMPLE_DTS (stream, sample) >= segment->media_start) {
    ...
    stream->time_position =
	 ...
	 GST_DEBUG_OBJECT (qtdemux, "%s_time_position:%"G_GUINT64_FORMAT" sample_timestamp:%" G_GUINT64_FORMAT" segment time: %" G_GUINT64_FORMAT, GST_PAD_NAME (stream->pad), stream->time_position, sample->timestamp, segment->time);
  } else {
...
next_segment:
  {
    GST_DEBUG_OBJECT (qtdemux, "segment %d Eended %d", stream->segment_index, stream->n_segments);

    if (stream->segment_index == stream->n_segments - 1) {
      /* are we at the end of the last segment, we're EOS */
	 GST_DEBUG_OBJECT(qtdemux,"RRRR set time postition to GST_CLOCK_TIME_NONE");
	   stream->total_timeposition = stream->time_position;
      //stream->time_position = GST_CLOCK_TIME_NONE;
    } else {
	 GST_DEBUG_OBJECT(qtdemux,"RRRR set time postition to segment->stop.time");
    ...
    GstBuffer * buf)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "3GPP timed text subtitle: %d/%" G_GSIZE_FORMAT "",
      nsize, map.size);
...
//if(qtdemux->mainbuf == NULL)
if(1)
{
  /* position reporting */
  if (qtdemux->segment.rate >= 0) {
    GST_DEBUG_OBJECT (qtdemux, "SYNC STREAM");
    qtdemux->segment.position = position;
    gst_qtdemux_sync_streams (qtdemux);
  }

  if (G_UNLIKELY (!stream->pad)) {
    //GST_DEBUG_LOG (qtdemux, "No output pad for stream, ignoring");
    gst_buffer_unref (buf);
    goto exit;
  }
  ...
  while (stream->buffers) {
    ...
    if (G_UNLIKELY (stream->discont)) {
      GST_DEBUG_OBJECT (qtdemux, "marking discont buffer");
  ...
  if (G_UNLIKELY (stream->need_process))
      GST_DEBUG_OBJECT (qtdemux, "process buffer");
  ...
  if (!keyframe) {
  GST_DEBUG_OBJECT (qtdemux,"PPPPPPPPPPad:%s_FALSE", GST_PAD_NAME (stream->pad));
    GST_BUFFER_FLAG_SET (buf, GST_BUFFER_FLAG_DELTA_UNIT);
    stream->on_keyframe = FALSE;
  } else {
  GST_DEBUG_OBJECT (qtdemux,"PPPPPPPPPPad:%s_TRUE", GST_PAD_NAME (stream->pad));
    stream->on_keyframe = TRUE;
  }
  GST_DEBUG_OBJECT (qtdemux,"PPPPPPPPPPad:%s", GST_PAD_NAME (stream->pad));
  ...
  if (stream->protected && stream->protection_scheme_type == FOURCC_cenc) {
    ...
    if (G_LIKELY (index >= 0 && index < info->crypto_info->len)) {
      ...
      GST_DEBUG_OBJECT (qtdemux, "attaching cenc metadata [%u/%u]", index,
...
GstFlowReturn
gst_mmtdemux_state_movie (GstQTDemux * qtdemux, GstBuffer *mdatbuf, guint64 mfuoffset, int mediaType, gint32 sample_number)
{
  GstFlowReturn ret = GST_FLOW_OK;
  //GstBuffer *buf = NULL;
  QtDemuxStream *stream;
...
//static GstClockTime old_pts[2];
	 //static int once;
	 //static GstClockTime pts_ref[2];
	 //static GstClockTime pts_ref;
   QtDemuxSample *sample;
  ...
  //min_time = G_MAXUINT64+G_MAXUINT64;
  index = -1;
	 if(qtdemux->n_streams != 2)
	 {
		  GST_DEBUG_OBJECT(qtdemux, "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBPPPPPPPAD mediaType: %d & n_streams: %d", mediaType, qtdemux->n_streams);
		  //stream->sample_index++;
		  return ret;
	 }
	 if(mdatbuf == NULL) 
	 {
		  GST_DEBUG_OBJECT(qtdemux, "AAAAAAABBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBPPPPPPPAD mediaType: %d & n_streams: %d", mediaType, qtdemux->n_streams);
	//	  goto next;
	//	  stream->sample_index++;
		  return ret;
	 }
  for (i = mediaType; i < qtdemux->n_streams; i++) {
	  //for (i = 0; i < qtdemux->n_streams; i++) {
	  GstClockTime position;
	  stream = qtdemux->streams[i];
	  sample = &stream->samples[stream->sample_index];
	  // if(qtdemux->mainbuf)
	  // position = stream->time_position+170666667;
	  // else
	  position = stream->time_position;
	  //GST_DEBUG_OBJECT (qtdemux,"%dAPPPPPPPPPPad:sample_index: %u  time_stamp: %"G_GUINT64_FORMAT "position: %"G_GUINT64_FORMAT" prev_position: %"G_GUINT64_FORMAT "n stream: %d", i, stream->sample_index, sample->timestamp, position, stream->time_position, qtdemux->n_streams);
	  // if( stream->pad == NULL)
	  // return GST_FLOW_OK;
	  GST_DEBUG_OBJECT (qtdemux,"%dAPPPPPPPPPPad:%s, sample_number: %"G_GUINT32_FORMAT" sample_index: %u  time_stamp: %"G_GUINT64_FORMAT "position: %"G_GUINT64_FORMAT" prev_position: %"G_GUINT64_FORMAT" Total samples: %d", i, GST_PAD_NAME (stream->pad), sample_number, stream->sample_index, sample->timestamp, position, stream->time_position, stream->n_samples);
	  //position = stream->time_position;

	  /* position of -1 is EOS */
	  if (position != GST_CLOCK_TIME_NONE && position < min_time) {
		  GST_DEBUG_OBJECT (qtdemux, "ABCD position is less then min time");
		  min_time = position;
		  index = mediaType;
	  }
	  index = mediaType;
  ...
  {
    GST_DEBUG_OBJECT (qtdemux, "HKMS..... OUT OF SAMPLES");
	  stream->samples[stream->n_samples - 1].timestamp +=
          stream->samples[stream->n_samples - 1].duration;
	  if(qtdemux->mainbuf)
	  mdatbuf->pts = mdatbuf->dts = stream->samples[stream->n_samples - 1].timestamp+2000000000;
	  else
	  mdatbuf->pts = mdatbuf->dts = stream->samples[stream->n_samples - 1].timestamp;
#if 0
	  if(qtdemux->mainbuf)
	  {
		  qtdemux->sample_reset = 1;
  ...
  {
    //int incr = sample_number-extracted_sample_number;
	// gst_qtdemux_advance_sample (qtdemux, stream);
    stream->sample_index = stream->sample_index+(sample_number-extracted_sample_number);
    //stream->sample_index++;
    GST_DEBUG_OBJECT (qtdemux, "different sample number..... %"G_GUINT32_FORMAT " :: %"G_GUINT32_FORMAT, sample_number, extracted_sample_number);
	 gst_mmtdemux_state_movie (qtdemux, mdatbuf, mfuoffset, mediaType, sample_number);
    return GST_FLOW_OK;
  }
	 if(stream->pad != NULL)
	 GST_DEBUG_OBJECT (qtdemux, "%s_BISMIL sample_size: %d, offset: %"G_GUINT64_FORMAT, GST_PAD_NAME (stream->pad), sample_size, offset);
  ...
  if (G_UNLIKELY (qtdemux->
    ...
    if (stream->subtype == FOURCC_vide && !keyframe) {
      GST_DEBUG_OBJECT (qtdemux, "Skipping non-keyframe on stream %d", index);
      goto next;
    }
  }
#endif
  ...
  GST_DEBUG_OBJECT (qtdemux, "reading %d bytes @ %" G_GUINT64_FORMAT, size,
      offset);

  //GST_DEBUG_OBJECT(qtdemux, "SHSHSHSHS sample offset:%"G_GUINT64_FORMAT " offset_end:%"G_GUINT64_FORMAT, mdatbuf->offset, mdatbuf->offset_end);
  if (size != sample_size) {
	 ...
	 if(!mediaType)
	 {
		 if(!once)
		 {
			 pts_ref[mediaType] = mdatbuf->pts -pts;
		    pts = mdatbuf->pts;
			 once = 1;
		 }
		 else
				pts +=pts_ref[mediaType];
		 //pts = pts_ref;
	 }
	 else
			 pts = mdatbuf->pts;
...
#if 0
		    if(!mediaType)
				old_pts[mediaType] += 16683000;
		    else
				old_pts[mediaType] += 21333;
		    dts = pts = old_pts[mediaType];
#endif
...
{
	ret = gst_pad_push (stream->pad, mdatbuf);
  //ret = gst_qtdemux_decorate_and_push_buffer (qtdemux, stream, mdatbuf,
  ...
  if (size != sample_size) {
    ...
    GstClockTime time_position = QTSTREAMTIME_TO_GSTTIME (stream,
        sample->timestamp +
        stream->offset_in_sample / CUR_STREAM (stream)->bytes_per_frame);
	 GST_DEBUG_OBJECT(qtdemux, "size is not equal to sample size");
  ...
  if (stream->offset_in_sample >= sample_size) {
	 GST_DEBUG_OBJECT (qtdemux, "SYXYXYX offset_in_sample is greater than sample size");
    gst_qtdemux_advance_sample (qtdemux, stream);
  }
  goto beach;
...
gst_qtdemux_loop_state_movie (GstQTDemux * qtdemux)
{
  ...
  if (qtdemux->fragmented_seek_pending) {
    GST_DEBUG_OBJECT (qtdemux, "pending fragmented seek");
    if (gst_qtdemux_do_fragmented_seek (qtdemux)) {
      GST_DEBUG_OBJECT (qtdemux, "fragmented seek done!");
      qtdemux->fragmented_seek_pending = FALSE;
    } else {
      GST_DEBUG_OBJECT (qtdemux, "fragmented seek still pending");
    }
  }
  ...
  for (i = 0; i < qtdemux->n_streams; i++) {
    ...
    stream = qtdemux->streams[i];
    position = stream->time_position;
	GST_DEBUG_OBJECT (qtdemux,"%dAPPPPPPPPPPad:%s position: %"G_GUINT64_FORMAT, i, GST_PAD_NAME (stream->pad), position);
  ...
  ret = gst_qtdemux_pull_atom (qtdemux, offset + stream->offset_in_sample,
      size, &buf);
  GST_DEBUG_OBJECT (qtdemux, "index: %d ABBBB offset_in_sample @ %" G_GUINT64_FORMAT" :: offset:%"G_GUINT64_FORMAT, index, buf->offset, buf->offset_end);
#if 0
	 if(qtdemux->mmdatbuffer[index])
		  qtdemux->mmdatbuffer[index] = gst_buffer_append (qtdemux->mmdatbuffer[index], buf);
	 else
		  qtdemux->mmdatbuffer[index] = buf;
#endif
  GST_DEBUG_OBJECT (qtdemux, "offset_in_sample @ %" G_GUINT32_FORMAT" :: offset:%"G_GUINT64_FORMAT, stream->offset_in_sample, offset);
  if (G_UNLIKELY (ret != GST_FLOW_OK))
    goto beach;
  GST_DEBUG_OBJECT (qtdemux, "UUUU size %d && sample_size @ %" G_GUINT32_FORMAT, size,
      sample_size);
  ...
  if (size != sample_size) {
    ...
    GstClockTime time_position = QTSTREAMTIME_TO_GSTTIME (stream,
        sample->timestamp +
        stream->offset_in_sample / CUR_STREAM (stream)->bytes_per_frame);
	 GST_DEBUG_OBJECT(qtdemux, "JJJJJJJ SIZE IS NOT EQUAL TO SAMPLE SIZE");
    if (time_position >= segment->media_start) {
      /* inside the segment, update time_position, looks very familiar to
       * GStreamer segments, doesn't it? */
      stream->time_position = (time_position - segment->media_start) +
...
gst_qtdemux_loop (GstPad * pad)
{
  ...
  cur_offset = qtdemux->offset;
  GST_DEBUG_OBJECT (qtdemux, "loop at position %" G_GUINT64_FORMAT " %x, state %s",
      cur_offset, (int)cur_offset, qt_demux_state_string (qtdemux->state));
  ...
  switch (qtdemux->state) {
    ...
    case QTDEMUX_STATE_HEADER:
	 //TODO MMTPARSING;
      ret = gst_qtdemux_loop_state_mmt_header (qtdemux);
...
pause:
  {
    ...
    GST_DEBUG_OBJECT (qtdemux, "pausing task, reason %s", reason);
    ...
    if (ret == GST_FLOW_EOS) {
      ...
      if (qtdemux->segment.flags & GST_SEEK_FLAG_SEGMENT) {
        ...
        if (qtdemux->segment.rate >= 0) {
          ...
          GST_DEBUG_OBJECT (qtdemux, "Sending segment done, at end of segment");
          ...
          GST_DEBUG_OBJECT (qtdemux, "Sending segment done, at start of segment");
          message = gst_message_new_segment_done (GST_OBJECT_CAST (qtdemux),
        ...
        GST_DEBUG_OBJECT (qtdemux, "Sending EOS at end of segment");
        event = gst_event_new_eos ();
        if (qtdemux->segment_seqnum != GST_SEQNUM_INVALID)
          gst_event_set_seqnum (event, qtdemux->segment_seqnum);
        gst_qtdemux_push_event (qtdemux, event);
...
static GstFlowReturn adapter_send_pkt(GstQTDemux *qtdemux, GstBuffer *buf, guint64 size) 
{
	guint64 PayldrLength;
	GstFlowReturn ret = GST_FLOW_OK;
   guint8 *data;
	...
	data = (guint8 *)gst_adapter_map (qtdemux->adapter, 16);
		PayldrLength=ParseMMTPHDR(data);
   gst_adapter_unmap (qtdemux->adapter);
	   
		GST_DEBUG_OBJECT (qtdemux, "INT_SIZEEEEEEEEE offset:%d, payload length: %"G_GUINT64_FORMAT "packet length: %"G_GUINT64_FORMAT, offset, PayldrLength, size);
		if(PayldrLength != 0x2)
		{
			data = (unsigned char *)gst_adapter_map (qtdemux->adapter, PayldrLength);
			qtdemux->packet_offset = qtdemux->offset+ 14;
			GST_DEBUG_OBJECT (qtdemux, "AV_PACKET offset:%d, payload length: %"G_GUINT64_FORMAT, offset, PayldrLength);
			qtdemux->ref_offset = qtdemux->packet_offset;
			gst_adapter_unmap (qtdemux->adapter);
			if((PayldrLength != 4) && (PayldrLength != 3))
				ParseMMTPV1Pack(qtdemux, data+14, buf);
		}
		else
		{
			data = (guint8 *)gst_adapter_map (qtdemux->adapter, size);
			SigOffset =ParseSignal(data);
			GST_DEBUG_OBJECT (qtdemux, "Bitstream offset: %"G_GUINT64_FORMAT "NNNNNNNNNNNNNN offset:%d",  qtdemux->offset, SigOffset);
			qtdemux->offset +=SigOffset;

			GST_DEBUG_OBJECT (qtdemux, " extract_initial_length_mmt length %"G_GUINT64_FORMAT, PayldrLength);
			gst_adapter_unmap (qtdemux->adapter);
			gst_adapter_flush (qtdemux->adapter, size);
		}

	   return ret;
}

static GstFlowReturn send_pkt(GstQTDemux *qtdemux, GstBuffer *buf, guint64 size) 
{
	guint64 PayldrLength;
	GstFlowReturn ret = GST_FLOW_OK;
	GstMapInfo map;
	...
	if (G_LIKELY (map.size >= 8)){
		...
		if(PayldrLength != 0x2)
		{
			...
			qtdemux->ref_offset = qtdemux->packet_offset;
			if((PayldrLength != 4) && (PayldrLength != 3))
				ParseMMTPV1Pack(qtdemux, map.data+14, buf);
		}
		else
		{
			SigOffset =ParseSignal(map.data);
...
gst_qtdemux_chain (GstPad * sinkpad, GstObject * parent, GstBuffer * inbuf)
{
  ...
  //return gst_qtdemux_loop_state_mmt_header (demux);
   ...
   //int SigOffset/*, packet_offset*/;
	...
	while (((gst_adapter_available (demux->adapter)) >= demux->neededbytes) &&
	   ...
	   data1 = (unsigned char*)data;
		  PayldrLength=ParseMMTPHDR(data1);
    if(PayldrLength != 0x2)
    {
...
static GstFlowReturn
bkpgst_qtdemux_process_adapter (GstQTDemux * demux, gboolean force)
{
  GstFlowReturn ret = GST_FLOW_OK;
    ...
    const guint8 * end)
{
  while (G_UNLIKELY (buf < end)) {
    ...
    if (G_UNLIKELY (buf + 4 > end)) {
      GST_DEBUG_OBJECT (qtdemux, "buffer overrun");
      break;
    }
    len = QT_UINT32 (buf);
    if (G_UNLIKELY (len == 0)) {
      GST_DEBUG_OBJECT (qtdemux, "empty container");
      break;
    }
    if (G_UNLIKELY (len < 8)) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "adding new node of len %d", len);
    ...
    GNode * xdxt)
{
  ...
  while (buf < end) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "%p %p", buf, end);
    ...
    switch (type) {
      case FOURCC_tCtH:
        ...
        GST_DEBUG_OBJECT (qtdemux, "parsing theora header");
        break;
      case FOURCC_tCt_:
        buffer = gst_buffer_new_and_alloc (size);
        gst_buffer_fill (buffer, 0, buf, size);
        stream->buffers = g_slist_append (stream->buffers, buffer);
        GST_DEBUG_OBJECT (qtdemux, "parsing theora comment");
        break;
      case FOURCC_tCtC:
        buffer = gst_buffer_new_and_alloc (size);
        gst_buffer_fill (buffer, 0, buf, size);
        stream->buffers = g_slist_append (stream->buffers, buffer);
        GST_DEBUG_OBJECT (qtdemux, "parsing theora codebook");
        break;
      default:
        GST_WARNING_OBJECT (qtdemux,
    ...
    guint length)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "qtdemux_parse buffer %p length %u", buffer, length);
  ...
  if (type->flags & QT_FLAG_CONTAINER) {
    ...
    switch (fourcc) {
      ...
      {
        if (node_length < 20) {
          GST_DEBUG_OBJECT (qtdemux, "skipping small stsd box");
          break;
        }
        GST_DEBUG_OBJECT (qtdemux,
      ...
      {
        if (node_length < 20) {
          GST_DEBUG_OBJECT (qtdemux, "skipping hmhd box");
          break;
        }
        GST_DEBUG_OBJECT (qtdemux,
            "parsing hmhd (hint handler table, hint description) atom");
			qtdemux_parse_hmhd (qtdemux, buffer, end - buffer);
        qtdemux_parse_container (qtdemux, node, buffer + 16, end);
        break;
      }
      case FOURCC_mp4a:
      ...
      case FOURCC_fLaC:
      {
        ...
        if (length < min_size) {
          GST_DEBUG_OBJECT (qtdemux, "skipping small %" GST_FOURCC_FORMAT " box",
              GST_FOURCC_ARGS (fourcc));
          break;
        }
      ...
      case FOURCC_mmth:
		{
			//qtdemux->mediaType=3;
			qtdemux->ismmth=1;
			GST_DEBUG_OBJECT (qtdemux, "111 .... box");
			GST_DEBUG_OBJECT (qtdemux, "MOSSA::::::: parsed mmth\n");
			qtdemux_parse_mmth (qtdemux, buffer+8, node_length);
			qtdemux_parse_container (qtdemux, node, buffer + 22, end);
			break;
		}
      default:
  ...
  GST_DEBUG_OBJECT (qtdemux, "parsed '%" GST_FOURCC_FORMAT "'",
      GST_FOURCC_ARGS (fourcc));
  return TRUE;
    ...
    QtDemuxStream * stream, GstTagList * list)
{
  ...
  if (stream->subtype == FOURCC_vide) {
    gchar *name = g_strdup_printf ("video_%u", qtdemux->n_video_streams);
	 GST_DEBUG_OBJECT (qtdemux, "QQQQQQQ stream subtype: video");
    ...
    gchar *name = g_strdup_printf ("audio_%u", qtdemux->n_audio_streams);
	 GST_DEBUG_OBJECT (qtdemux, "QQQQQQQ stream subtype: audio");
    ...
    guint64 * length, guint32 fourcc)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "finding fourcc %" GST_FOURCC_FORMAT " at offset %"
      G_GUINT64_FORMAT, GST_FOURCC_ARGS (fourcc), *offset);
  GST_DEBUG_OBJECT (qtdemux, "MMMM finding fourcc %" GST_FOURCC_FORMAT " at offset %"
      G_GUINT64_FORMAT, GST_FOURCC_ARGS (fourcc), *offset);
...
qtdemux_add_fragmented_samples (GstQTDemux * qtdemux)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "SSADA");
...
qtdemux_stbl_init (GstQTDemux * qtdemux, QtDemuxStream * stream, GNode * stbl)
{
  ...
  if (!gst_byte_reader_skip (&stream->stts, 1 + 3) ||
    ...
    goto corrupt_file;
  GST_DEBUG_OBJECT (qtdemux, "%u timestamp blocks", stream->n_sample_times);
  ...
  if (!qt_atom_parser_has_chunks (&stream->stts, stream->n_sample_times, 8)) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "overriding to %u timestamp blocks",
  ...
  GST_DEBUG_OBJECT (qtdemux, "SSSSSSSSAMPLES n_samples %u",
...
qtdemux_parse_samples (GstQTDemux * qtdemux, QtDemuxStream * stream, guint32 n)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "parsing samples for stream fourcc %"
  ...
  if (!stream->chunks_are_samples) {
    ...
    if (stream->sample_size == 0) {
      ...
      for (cur = first; cur <= last; cur++) {
        ...
        GST_DEBUG_OBJECT (qtdemux, "sample %d has size %u",
            (guint) (cur - samples), cur->size);
      }
    } else {
      /* samples have the same size */
      GST_DEBUG_OBJECT (qtdemux, "all samples have size %u", stream->sample_size);
      for (cur = first; cur <= last; cur++)
  ...
  for (i = stream->stsc_index; i < n_samples_per_chunk; i++) {
    ...
    if (stream->stsc_chunk_index >= stream->last_chunk
      ...
      if (G_UNLIKELY (stream->first_chunk == 0))
		{
		  GST_DEBUG_OBJECT(qtdemux, "CORRUPTED_FILE");
			goto corrupt_file;
		}
    ...
    if (stream->chunks_are_samples) {
      ...
      for (j = stream->stsc_chunk_index; j < last_chunk; j++) {
        ...
        GST_DEBUG_OBJECT (qtdemux, "Created entry %d with offset "
      ...
      for (j = stream->stsc_chunk_index; j < last_chunk; j++) {
        ...
        for (k = stream->stsc_sample_index; k < samples_per_chunk; k++) {
          GST_DEBUG_OBJECT (qtdemux, "creating entry %d with offset %"
...
done2:
  {
    ...
    for (i = stream->stts_index; i < n_sample_times; i++) {
      ...
      if (stream->stts_sample_index >= stream->stts_samples
        ...
        GST_DEBUG_OBJECT (qtdemux, "block %d, %u timestamps, duration %u",
...
qtdemux_parse_trak (GstQTDemux * qtdemux, GNode * trak)
{
  ...
  if(!trak_count || (mediaType != qtdemux->mediaType))
	 {
		  trak_count = 1;
	 }
	 else
		  trak_count = 2;
	 mediaType = qtdemux->mediaType;
  ...
  if (!gst_byte_reader_skip (&tkhd, value_size * 2) ||
    ...
    goto corrupt_file;
  GST_DEBUG_OBJECT (qtdemux, "MM, parse_trak: tkhd_version:%d, tkhd_flags:%d, trak_id: %d mediaType: %d\n", tkhd_version, tkhd_flags, track_id, qtdemux->mediaType);
#if 0
	 if( (track_id != 2) && (qtdemux->n_streams == 2) )
	 {
		 for (n = 0; n < qtdemux->n_streams; n++) {
			 gst_qtdemux_stream_free (qtdemux, qtdemux->streams[n]);
			 qtdemux->streams[n] = NULL;
  ...
  if (!qtdemux->got_moov) {
  //if (!qtdemux->moov_detected) {
	 //TODO: BY SAFWAN... JUST FOR NOW
	 //if(track_id == 2 /*&& qtdemux->ismmth*/)
	 //if(track_id == 2 && qtdemux->ismmth)
	 if(trak_count == 2)
	 {
		 GST_DEBUG_OBJECT (qtdemux, "ZZZZZZ skip track for Hint Handler");
		 goto skip_track;
	 }
	 if(qtdemux->mediaType == 2 /*&& qtdemux->ismmth*/)
		  track_id = 2;
  GST_DEBUG_OBJECT (qtdemux, "didnt get_moov");
    if (qtdemux_find_stream (qtdemux, track_id))
	 ...
	 // if(track_id == 2 /*&& qtdemux->ismmth*/)
	  ...
	  if(qtdemux->mediaType == 2)
		  track_id = 2;
	  GST_DEBUG_OBJECT (qtdemux, " got_moov");
	  stream = qtdemux_find_stream (qtdemux, track_id);
	  if (!stream) {
  ...
  GST_DEBUG_OBJECT (qtdemux, "track[tkhd] version/flags/id: 0x%02x/%06x/%u",
  ...
  GST_DEBUG_OBJECT (qtdemux, "track version/flags: %08x", version);
  ...
  GST_DEBUG_OBJECT (qtdemux, "track timescale: %" G_GUINT32_FORMAT,
      stream->timescale);
  GST_DEBUG_OBJECT (qtdemux, "track duration: %" G_GUINT64_FORMAT,
      stream->duration);
  GST_DEBUG_OBJECT (qtdemux, "track language code/id: 0x%04x/%s",
  ...
  GST_DEBUG_OBJECT (qtdemux, "KKKK w: %u h: %u   ", w, h);
  ...
  GST_DEBUG_OBJECT (qtdemux, "stsd len:           %d", stsd_len);
  GST_DEBUG_OBJECT (qtdemux, "stsd entry count:   %u", stsd_entry_count);
  ...
  for (stsd_index = 0; stsd_index < stsd_entry_count; stsd_index++) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "stsd type:          %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (entry->fourcc));
    GST_DEBUG_OBJECT (qtdemux, "stsd type len:      %d", len);
    ...
    if (stream->subtype == FOURCC_vide) {
      ...
      GST_DEBUG_OBJECT (qtdemux, "width %d, height %d, bps %d, color table id %d",
      ...
      if (entry->color_table_id) {
        ...
        GST_DEBUG_OBJECT (qtdemux, "start %d, end %d, palette_count %d",
      ...
      GST_DEBUG_OBJECT (qtdemux, "frame count:   %u",
      ...
      GST_DEBUG_OBJECT (qtdemux, "version/rev:      %08x", version);
      GST_DEBUG_OBJECT (qtdemux, "vendor:           %08x",
          QT_UINT32 (stsd_entry_data + offset + 4));
      GST_DEBUG_OBJECT (qtdemux, "n_channels:       %d", entry->n_channels);
      GST_DEBUG_OBJECT (qtdemux, "sample_size:      %d", samplesize);
      GST_DEBUG_OBJECT (qtdemux, "compression_id:   %d", compression_id);
      GST_DEBUG_OBJECT (qtdemux, "packet size:      %d",
          QT_UINT16 (stsd_entry_data + offset + 14));
      GST_DEBUG_OBJECT (qtdemux, "sample rate:      %g", entry->rate);
      ...
      if (version == 0x00010000) {
        ...
        switch (fourcc) {
          ...
          default:
          {
            ...
            GST_DEBUG_OBJECT (qtdemux, "samples/packet:   %d",
                entry->samples_per_packet);
            GST_DEBUG_OBJECT (qtdemux, "bytes/packet:     %d",
                entry->bytes_per_packet);
            GST_DEBUG_OBJECT (qtdemux, "bytes/frame:      %d",
                entry->bytes_per_frame);
            GST_DEBUG_OBJECT (qtdemux, "bytes/sample:     %d",
            ...
            if (!entry->sampled && entry->bytes_per_packet) {
              ...
              GST_DEBUG_OBJECT (qtdemux, "samples/frame:    %d",
        ...
        GST_DEBUG_OBJECT (qtdemux, "Sound sample description Version 2");
        GST_DEBUG_OBJECT (qtdemux, "sample rate:        %g", entry->rate);
        GST_DEBUG_OBJECT (qtdemux, "n_channels:         %d", entry->n_channels);
        GST_DEBUG_OBJECT (qtdemux, "bits/channel:       %d",
            QT_UINT32 (stsd_entry_data + offset + 20));
        GST_DEBUG_OBJECT (qtdemux, "format flags:       %X",
            QT_UINT32 (stsd_entry_data + offset + 24));
        GST_DEBUG_OBJECT (qtdemux, "bytes/packet:       %d",
            QT_UINT32 (stsd_entry_data + offset + 28));
        GST_DEBUG_OBJECT (qtdemux, "LPCM frames/packet: %d",
          ...
          while (len >= 8) {
            ...
            switch (QT_FOURCC (wfex_data + 4)) {
              case GST_MAKE_FOURCC ('w', 'f', 'e', 'x'):
              {
                ...
                GST_DEBUG_OBJECT (qtdemux, "Found wfex box in stsd:");
                GST_DEBUG_OBJECT (qtdemux, "FormatTag = 0x%04x, Channels = %u, "
      ...
      mp4a = qtdemux_tree_get_child_by_index (stsd, stsd_index);
	   GST_DEBUG_OBJECT(qtdemux,"stsd_index: %u\n", stsd_index);
      if (!stream->protected) {
      } else {
        if (QTDEMUX_TREE_NODE_FOURCC (mp4v) != FOURCC_encv) {
          mp4v = NULL;
      ...
      switch (fourcc) {
        case FOURCC_mp4s:
        {
          ...
          if (esds == NULL) {
            /* Invalid STSD */
            GST_DEBUG_OBJECT (qtdemux, "Skipping invalid stsd: no esds child");
            break;
          }
    ...
    remaining_stsd_len -= len;
	 ...
	 GST_DEBUG_OBJECT (qtdemux, "STBL INIT");
  /* collect sample information */
  if (!qtdemux_stbl_init (qtdemux, stream, stbl))
  {
	 GST_DEBUG_OBJECT (qtdemux, "STBL SAMPLE FAILED");
	  goto samples_failed;
  }

	 GST_DEBUG_OBJECT (qtdemux, "STBL INIT LATER");
  if (qtdemux->fragmented) {
    guint64 offset;
	 GST_DEBUG_OBJECT (qtdemux, "STBL samples:%"G_GUINT32_FORMAT, stream->n_samples);
  ...
  if (qtdemux->n_streams >= GST_QTDEMUX_MAX_STREAMS)
    ...
    GST_DEBUG_OBJECT (qtdemux, "TTTTTTT n_streams is now %d", qtdemux->n_streams);
...
qtdemux_prepare_streams (GstQTDemux * qtdemux)
{
  ...
  for (i = 0; ret == GST_FLOW_OK && i < qtdemux->n_streams; i++) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "stream %d, id %d, fourcc %" GST_FOURCC_FORMAT "no of stream %d",
        i, stream->track_id, GST_FOURCC_ARGS (CUR_STREAM (stream)->fourcc), qtdemux->n_streams);
#if 0
    if (qtdemux->fragmented) {
      ...
      while (stream->n_samples == 0)
        if ((ret = qtdemux_add_fragmented_samples (qtdemux)) != GST_FLOW_OK)
		  {
		   GST_DEBUG_OBJECT (qtdemux, "added fragmented samples from streams");
          break;
		  }
      GST_OBJECT_UNLOCK (qtdemux);
    } else {
    ...
    if (stream->n_samples > 0 && stream->stbl_index >= 0) {
      ...
      GST_DEBUG_OBJECT (qtdemux, "stream %d first duration %u",
...
qtdemux_expose_streams (GstQTDemux * qtdemux)
{
  ...
  for (i = 0; i < qtdemux->n_streams; i++) {
    ...
    if (!gst_qtdemux_add_stream (qtdemux, stream, list))
	 {
	   GST_DEBUG_OBJECT (qtdemux, "exposed streams Failed");
      return GST_FLOW_ERROR;
	 }
  }
...
qtdemux_parse_udta (GstQTDemux * qtdemux, GstTagList * taglist, GNode * udta)
{
  ...
  if (meta != NULL) {
    ...
    if (ilst == NULL) {
      GST_DEBUG_OBJECT (qtdemux, "no ilst");
      return;
    }
  } else {
    ilst = udta;
    GST_DEBUG_OBJECT (qtdemux, "no meta so using udta itself");
  }
  ...
  if (rmra) {
    ...
    while (rmda) {
      ...
      if ((rmdr = qtdemux_tree_get_child_by_type (rmda, FOURCC_rmdr))) {
        ...
        GST_DEBUG_OBJECT (qtdemux, "data rate atom, required bitrate = %u",
            ref.min_req_bitrate);
      }
...
qtdemux_add_container_format (GstQTDemux * qtdemux, GstTagList * tags)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "mapped %" GST_FOURCC_FORMAT " to '%s'",
...
qtdemux_parse_tree (GstQTDemux * qtdemux)
{
  ...
  if (mvhd == NULL) {
    GST_DEBUG_OBJECT (qtdemux, "No mvhd node found, looking for redirects.");
  ...
  if (udta) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "No udta node found.");
  }
  ...
  if (udta) {
    ...
    GST_DEBUG_OBJECT (qtdemux, "No meta node found.");
  }
